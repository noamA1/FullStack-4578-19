...
let bookOne = new Book("What If It's Us", "Becky Albertalli, Adam Silvera", "HarperCollins Publishers", 11.49);
let bookTwo = new Book("Game of Thrones (A Song of Ice and Fire #1)", "George R. R. Martin", "Random House Publishing Group", 15.99);
let bookThree = new Book("Harry Potter and the Chamber of Secrets", "J. K. Rowling", "Scholastic, Inc.", 9.99);
...
for (const element of getVatButtonElement) {
    element.addEventListener("click", () => {
        ...
        const vatTableDataElement = document.querySelector(`#vat-${arrayOfBooks.indexOf(book)}`);
        vatTableDataElement.innerHTML = `${vatOfBook}&#36;`;
    });
}
// #2: perfumes store
let arrayOfPerfumes;
const getPerfumeVatButtonElement = document.getElementsByClassName("perfume-vat-button");
let perfumeOne = new Perfume("Alien Goddess Eau de Parfum", "Mugler", "100ML", true, 135);
...
for (const element of getPerfumeVatButtonElement) {
    element.addEventListener("click", () => {
        ...
        const vatTableDataElement = document.querySelector(`#perfume-vat-${arrayOfPerfumes.indexOf(perfume)}`);
        vatTableDataElement.innerHTML = `${vatOfPerfume}&#36;`;
    });
}
// #2.1: Employees
let employeeOne = new Employee("Patricia", "Lebsack", {
    street: "Hoeger Mall",
    city: "South Elvis",
    zipcode: "53919-4257",
}, 30000);
let employeeTwo = new Employee("Ervin", "Howell", {
...
const getFirstTaxButtonElement = document.querySelector(`#tax-${employeeOne.firstName}`);
const getSecondTaxButtonElement = document.querySelector(`#tax-${employeeTwo.firstName}`);
getFirstTaxButtonElement.addEventListener("click", () => {
    let tax = employeeOne.getTax();
    const employeeCardList = document.querySelector(`#${employeeOne.firstName}-list`);
...
getSecondTaxButtonElement.addEventListener("click", () => {
    ...
    const employeeCardList = document.querySelector(`#${employeeTwo.firstName}-list`);
    let taxItem = document.createElement("li");
    taxItem.innerHTML = `<strong>Tax: </strong> ${tax} &#36;`;
    employeeCardList.appendChild(taxItem);
